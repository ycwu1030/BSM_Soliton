...
int main(int argc, char const *argv[]) {
    ...
    for (int rid = -30; rid <= 40; rid += 1) {
        ...
        model.DumpFullSolution(sol.R, sol.Phi, sol.dPhi_1D, fname);
        out << r << " " << model.GetTotalEnergy(sol.R, sol.Phi, sol.dPhi_1D) << " "
            << model.GetWallWidth(sol.R, sol.Phi, sol.dPhi_1D) << endl;
    }
    double r = 3.0 / 4.0;
    model.Set_Potential_Parameters(r);
    VVD pts_init;
