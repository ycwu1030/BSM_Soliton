...
double find_min_arg_gsl_wraper(root_func func, void *params, double x_max, double x_min, double epsabs,
                               double epsfrac) {
    int status;
    int iter = 0, max_iter = 100;
...
GSL_Spline_Inter::GSL_Spline_Inter() { _acc = gsl_interp_accel_alloc(); }
...
double GSL_Spline_Inter::valAt(double xi, int deri) {
    ...
    // ! GSL does not support extrapolation, so we extrapolate it using the simplest method (mirror about the end point)
    // by ourself ! But we don't extrapolate too much, otherwise, the user should provide more points to do interpolate
    double x_pick = xi;
    if (xi < _xmin) {
    ...
    switch (deri) {
        ...
        default:
            cout << "[Warning: GSL_Spline_Inter::valAt] Wrong derivative order (" << deri
                 << "), which should be <=2. Use 0 instead" << endl;
            res = gsl_spline_eval(_spline, x_pick, _acc);
            break;
    }
    if (xi < _xmin) {
...
GSL_Multi_Spline_Inter::GSL_Multi_Spline_Inter() { _acc = gsl_interp_accel_alloc(); }
