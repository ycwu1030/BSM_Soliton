...
void Kink1D::findScales() {
    ...
    V_scaled = [&](double field_hat) { return V(field_hat * phiscale); };
    dV_scaled = [&](double field_hat) { return dV(field_hat * phiscale); };
    d2V_scaled = [&](double field_hat) { return d2V(field_hat * phiscale); };
...
tuple<double, VD, CONVERGENCETYPE> Kink1D::integrateProfile(VD y0, VD y_desired, double dr0, double phi_eps_rel_,
    ...
    VD y_scale;
    VD y_inter(2);
    int y_direction_sign =
        y[0] < phi_final ? 1 : -1;  // This is the direction of the solution, if y[0] < phi_final, it means, y[0] needs
                                    // to be increased, while y[0] > phi_final means y[0] needs to be decreased.
    ...
    while (true) {
        ...
        if (y_cache[1] * y_direction_sign < 0) {
            ...
            if (cubicInterpolation(0, &inter_param) * cubicInterpolation(1, &inter_param) > 0) {
                cout << "In integrateProfile: Error in handling UNDERSHOOT case, the two end points suppose to have "
                        "opposite sign, however they have the same sign, just break"
                     << endl;
                r = r_cache;
                y = y_cache;
                break;
        ...
        if ((y_cache[0] - phi_final) * y_direction_sign > 0) {
            ...
            if (cubicInterpolation(0, &inter_param) * cubicInterpolation(1, &inter_param) > 0) {
                cout << "In integrateProfile: Error in handling OVERSHOOT case, the two end points suppose to have "
                        "opposite sign, however they have the same sign, just break"
                     << endl;
                r = r_cache;
                y = y_cache;
...
tuple<VD, VD, VD, VD> Kink1D::findProfile(double dphi0_tol_rel, double phi_tol_rel, int npoints, double rmax) {
    ...
    if (ctype == UNDERSHOOT) {
        cout << "In findProfile: converge in absolute minimum side but undershoot in meta-stable side. Either the "
                "potential has a Z2 symmetry but the RK precsion is not enough, or something wrong in the potential!"
             << endl;
    }
