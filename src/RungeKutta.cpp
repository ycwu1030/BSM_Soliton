 ...
 * @LastEditTime : 2020-02-05 14:24:06
 */
...
void RungeKutta::_RKQC_SingleStep(double &X, VD &Y, VD dY, double step_size_guess, double eps, VD Y_Scale, double &step_size_did, double &step_size_further)
{
    ...
    double error_max = 0;
    double min_step_size = 1e-3*step_size_guess; // Since we will adapt the step size according to the estimated error, we need to terminate such operate at some point, otherwise, we will stuck at here for some case.
    double max_step_size = 4*step_size_guess; // We will enlarge the step size, when we are within the precision. But we need to limit it within a reasonable range.
    ...
    while (true)
    {
        ...
        y_temp = Y - y_temp;
        error_temp = y_temp/Y_Scale;
        error_max = *max_element(error_temp.begin(),error_temp.end(),abs_compare);
        error_max /= eps;
        if (error_max <= 1.0)
        {
        ...
        if (step_size_temp < min_step_size)
        {
            step_size_did = step_size;
