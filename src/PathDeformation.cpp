...
SplinePath::SplinePath(VVD pts, VnD V_, int V_spline_samples, bool extend_to_minima, bool re_eval_distances) {
    ...
    VD x = linspace(0.0, _L, V_spline_samples);
...
VD SplinePath::pts_at_dist(double x, int deri) { return _path_Inter.valAt(x, deri); }
...
Deformation_Spline::Deformation_Spline(VVD phi, VD dphidr, dVnD dV, int ncoeffs, int K, double v2min, bool fix_start,
                                       bool fix_end, bool save_all_steps)
    : _fitter(K, ncoeffs) {
...
tuple<double, bool, double> Deformation_Spline::step(double last_step, double max_step, double min_step,
                                                     double reverseCheck, double stepIncrease, double stepDecrease,
                                                     bool checkAfterFit) {
    VVD F, dV;
    tie(F, dV) = forces();
    ...
    if (reverseCheck < 1 && _F_last.size() != 0) {
        ...
        if (reversed > (FdotFlast.size()) * reverseCheck) {
            ...
            // No that many points change the sign, so we want to increase the step size a bit to accelerate the
            // procedure
...
Deformation_Status Deformation_Spline::deformPath(double start_step, double fRatioConv, double converge_0,
                                                  double fRatioIncrease, int maxiter,
                                                  function<bool(Deformation_Spline *)> callback) {
...
KinknD fullKink(VVD pts_init, VnD V_in, dVnD dV_in, int maxiter, double fixEndCutoff, bool save_all_steps,
                int V_spline_samples) {
    ...
    for (size_t num_iter = 0; num_iter < maxiter; num_iter++) {
        ...
        // * We don't allow more steps, since during deformPath() we didn't re-calculate the 1D tunneling profile. With
        // more steps, the convergence status should be recalculated.
