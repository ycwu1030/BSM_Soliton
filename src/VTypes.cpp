...
bool CloseQ(double x1, double x2) { return abs(x1 - x2) < 1e-2; }
bool CloseQ(VD x1, VD x2, double close_criteria) {
    VD difs = x1 - x2;
    double distance = sqrt(difs * difs);
    return distance < close_criteria;
}
...
double GetMag(double vre, double vim) { return sqrt(vre * vre + vim * vim); }
